...
Definition globals : string := "ethereum.istanbul.trie".
...
Axiom ethereum_constantinople_imports :
  AreImported globals "ethereum.constantinople" [ "trie" ].
...
Axiom ethereum_istanbul_blocks_imports :
  AreImported globals "ethereum.istanbul.blocks" [ "Receipt" ].

Axiom ethereum_istanbul_fork_types_imports :
  AreImported globals "ethereum.istanbul.fork_types" [ "Account"; "Address"; "Root"; "encode_account" ].

Axiom ethereum_istanbul_transactions_imports :
  AreImported globals "ethereum.istanbul.transactions" [ "Transaction" ].
...
Definition Node : Value.t := M.run ltac:(M.monadic (
  M.get_subscript (|
    ...
    make_tuple [ M.get_name (| globals, "Account" |); M.get_name (| globals, "Bytes" |); M.get_name (| globals, "Transaction" |); M.get_name (| globals, "Receipt" |); M.get_name (| globals, "Uint" |); M.get_name (| globals, "U256" |); Constant.None_ ]
  |)
)).
...
Definition V : Value.t := M.run ltac:(M.monadic (
  M.call (|
    ...
    make_list [
      ...
      M.get_subscript (|
        M.get_name (| globals, "Optional" |),
        M.get_name (| globals, "Transaction" |)
      |);
      M.get_subscript (|
        M.get_name (| globals, "Optional" |),
