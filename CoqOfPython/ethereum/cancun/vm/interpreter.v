...
Axiom state_mark_account_created :
  IsGlobalAlias globals state.globals "mark_account_created".
...
Axiom exceptions_InvalidContractPrefix :
  IsGlobalAlias globals exceptions.globals "InvalidContractPrefix".
...
Definition process_create_message : Value.t -> Value.t -> M :=
  fun (args kwargs : Value.t) => ltac:(M.monadic (
    ...
    make_list [
      ...
      M.get_field (| M.get_name (| globals, "env" |), "transient_storage" |)
    ],
    make_dict []
  |) in
    let _ := M.call (|
