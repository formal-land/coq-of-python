...
Axiom state_get_transient_storage :
  IsGlobalAlias globals state.globals "get_transient_storage".
Axiom state_set_storage :
  IsGlobalAlias globals state.globals "set_storage".
Axiom state_set_transient_storage :
  IsGlobalAlias globals state.globals "set_transient_storage".
...
Definition sload : Value.t -> Value.t -> M :=
  fun (args kwargs : Value.t) => ltac:(M.monadic (
    ...
    let _ :=
      (* if *)
      M.if_then_else (|
        Compare.in (| make_tuple [ M.get_field (| M.get_field (| M.get_name (| globals, "evm" |), "message" |), "current_target" |); M.get_name (| globals, "key" |) ], M.get_field (| M.get_name (| globals, "evm" |), "accessed_storage_keys" |) |),
      (* then *)
      ltac:(M.monadic (
        let _ := M.call (|
...
Definition sstore : Value.t -> Value.t -> M :=
  ...
  |) in
    ...
    let _ :=
      (* if *)
      M.if_then_else (|
        Compare.not_in (| make_tuple [ M.get_field (| M.get_field (| M.get_name (| globals, "evm" |), "message" |), "current_target" |); M.get_name (| globals, "key" |) ], M.get_field (| M.get_name (| globals, "evm" |), "accessed_storage_keys" |) |),
      (* then *)
      ltac:(M.monadic (
        let _ := M.call (|
    M.get_field (| M.get_field (| M.get_name (| globals, "evm" |), "accessed_storage_keys" |), "add" |),
...
Definition tload : Value.t -> Value.t -> M :=
  ...
  |) in
    let value :=
      M.call (|
        M.get_name (| globals, "get_transient_storage" |),
        make_list [
          M.get_field (| M.get_field (| M.get_name (| globals, "evm" |), "env" |), "transient_storage" |);
          M.get_field (| M.get_field (| M.get_name (| globals, "evm" |), "message" |), "current_target" |);
          M.get_name (| globals, "key" |)
        ],
        make_dict []
...
Definition tstore : Value.t -> Value.t -> M :=
  ...
  |) in
    let _ := M.call (|
    M.get_name (| globals, "set_transient_storage" |),
    make_list [
      M.get_field (| M.get_field (| M.get_name (| globals, "evm" |), "env" |), "transient_storage" |);
      M.get_field (| M.get_field (| M.get_name (| globals, "evm" |), "message" |), "current_target" |);
      M.get_name (| globals, "key" |);
