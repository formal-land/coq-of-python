...
Axiom gas_GAS_BLOBHASH_OPCODE :
  IsGlobalAlias globals gas.globals "GAS_BLOBHASH_OPCODE".
...
Axiom gas_calculate_blob_gas_price :
  IsGlobalAlias globals gas.globals "calculate_blob_gas_price".
...
Definition blob_hash : Value.t -> Value.t -> M :=
  fun (args kwargs : Value.t) => ltac:(M.monadic (
    ...
    make_list [
      M.get_name (| globals, "evm" |);
      M.get_name (| globals, "GAS_BLOBHASH_OPCODE" |)
    ],
    make_dict []
  |) in
    let _ :=
      (* if *)
      M.if_then_else (|
        Compare.lt (| M.get_name (| globals, "index" |), M.call (|
          M.get_name (| globals, "len" |),
          make_list [
  ...
  |) in
    let blob_base_fee :=
      M.call (|
        ...
        make_list [
          M.get_field (| M.get_field (| M.get_name (| globals, "evm" |), "env" |), "excess_blob_gas" |)
        ],
        make_dict []
      |) in
    let _ := M.call (|
    ...
    make_list [
      ...
      M.call (|
        M.get_name (| globals, "U256" |),
        make_list [
          M.get_name (| globals, "blob_base_fee" |)
        ],
        make_dict []
      |)
    ],
