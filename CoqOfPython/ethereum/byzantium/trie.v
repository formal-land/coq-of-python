...
Axiom ethereum_spurious_dragon___init___trie :
  IsGlobalAlias globals ethereum.spurious_dragon.__init__.globals "trie".
...
Axiom ethereum_byzantium_fork_types_Account :
  IsGlobalAlias globals ethereum.byzantium.fork_types.globals "Account".
...
Axiom ethereum_byzantium_fork_types_encode_account :
  IsGlobalAlias globals ethereum.byzantium.fork_types.globals "encode_account".
      ...
      let _ :=
        ...
        M.if_then_else (|
          BoolOp.or (|
            ...
            ltac:(M.monadic (
              Compare.not_eq (|
                M.get_subscript (| M.get_name (| globals, "a" |), M.get_name (| globals, "i" |) |),
                M.get_subscript (| M.get_name (| globals, "b" |), M.get_name (| globals, "i" |) |)
              |)
            ))
          |),
        (* then *)
...
Definition patricialize : Value.t -> Value.t -> M :=
  fun (args kwargs : Value.t) => ltac:(M.monadic (
      ...
      ltac:(M.monadic (
        let leaf :=
          M.call (|
            M.get_name (| globals, "LeafNode" |),
            make_list [
              M.get_subscript (| M.get_name (| globals, "arbitrary_key" |), M.slice (| M.get_name (| globals, "level" |), Constant.None_ |) |);
    ...
    let substring :=
      M.get_subscript (| M.get_name (| globals, "arbitrary_key" |), M.slice (| M.get_name (| globals, "level" |), Constant.None_ |) |) in
    let prefix_length :=
      M.call (|
        M.get_name (| globals, "len" |),
        make_list [
        ...
        M.call (|
          ...
          make_list [
            ...
            M.call (|
              ...
              make_list [
                M.get_name (| globals, "substring" |);
                M.get_subscript (| M.get_name (| globals, "key" |), M.slice (| M.get_name (| globals, "level" |), Constant.None_ |) |)
              ],
              make_dict []
            |)
          ],
    ...
    let _ :=
      ...
      ltac:(M.monadic (
        let prefix :=
          M.get_subscript (| M.get_name (| globals, "arbitrary_key" |), M.slice (| M.get_name (| globals, "level" |), BinOp.add (|
      ...
      let _ :=
        ...
        ltac:(M.monadic (
          let _ :=
            ...
            (* then *)
            ltac:(M.monadic (
              let _ := M.raise (| Some(M.get_name (| globals, "AssertionError" |)) |) in
              M.pure Constant.None_
            (* else *)
            )), ltac:(M.monadic (
              M.pure Constant.None_
