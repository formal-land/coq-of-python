...
Require ethereum.paris.
Axiom ethereum_paris_trie :
  IsGlobalAlias globals ethereum.paris.globals "trie".
...
Definition Node : Value.t := M.run ltac:(M.monadic (
  M.get_subscript (| M.get_name (| globals, "Union" |), make_tuple [ M.get_name (| globals, "Account" |); M.get_name (| globals, "Bytes" |); M.get_name (| globals, "LegacyTransaction" |); M.get_name (| globals, "Receipt" |); M.get_name (| globals, "Uint" |); M.get_name (| globals, "U256" |); M.get_name (| globals, "Withdrawal" |); Constant.None_ ] |)
)).
...
Definition V : Value.t := M.run ltac:(M.monadic (
  M.call (|
    ...
    make_list [
      ...
      M.get_subscript (| M.get_name (| globals, "Optional" |), M.get_subscript (| M.get_name (| globals, "Union" |), make_tuple [ M.get_name (| globals, "Withdrawal" |); M.get_name (| globals, "Bytes" |) ] |) |);
...
(* At stmt: unsupported node type: AnnAssign *)
    let _ :=
      (* if *)
      M.if_then_else (|
        Compare.is (| M.get_name (| globals, "node" |), Constant.None_ |),
      (* then *)
      ltac:(M.monadic (
        let unencoded :=
          ...
          ltac:(M.monadic (
            let unencoded :=
              make_tuple [ M.call (|
                ...
                make_list [
                  M.get_field (| M.get_name (| globals, "node" |), "rest_of_key" |);
                  Constant.bool true
                ],
                make_dict []
              ...
              ltac:(M.monadic (
                let unencoded :=
                  make_tuple [ M.call (|
                    M.get_name (| globals, "nibble_list_to_compact" |),
                    make_list [
                      M.get_field (| M.get_name (| globals, "node" |), "key_segment" |);
                      Constant.bool false
                    ],
                    make_dict []
...
Definition encode_node : Value.t -> Value.t -> M :=
  fun (args kwargs : Value.t) => ltac:(M.monadic (
    ...
    let _ :=
      ...
      (* then *)
      ltac:(M.monadic (
        let _ := M.assert (| Compare.is_not (| M.get_name (| globals, "storage_root" |), Constant.None_ |) |) in
        let _ := M.return_ (|
          M.call (|
            M.get_name (| globals, "encode_account" |),
            make_list [
              M.get_name (| globals, "node" |);
        ...
        let _ :=
          ...
          M.if_then_else (|
            M.call (|
              ...
              make_list [
                M.get_name (| globals, "node" |);
                make_tuple [ M.get_name (| globals, "LegacyTransaction" |); M.get_name (| globals, "Receipt" |); M.get_name (| globals, "Withdrawal" |); M.get_name (| globals, "U256" |) ]
              ],
              make_dict []
            |),
          (* then *)
              ...
              M.call (|
                ...
                make_list [
                  M.call (|
                    ...
                    make_list [
                      M.get_field (| M.get_name (| globals, "rlp" |), "RLP" |);
                      M.get_name (| globals, "node" |)
                    ],
                    make_dict []
                  |)
...
Definition trie_set : Value.t -> Value.t -> M :=
  fun (args kwargs : Value.t) => ltac:(M.monadic (
    ...
    let _ :=
      (* if *)
      M.if_then_else (|
        Compare.eq (| M.get_name (| globals, "value" |), M.get_field (| M.get_name (| globals, "trie" |), "default" |) |),
      (* then *)
      ltac:(M.monadic (
        let _ :=
          (* if *)
          M.if_then_else (|
            Compare.in (| M.get_name (| globals, "key" |), M.get_field (| M.get_name (| globals, "trie" |), "_data" |) |),
          (* then *)
          ltac:(M.monadic (
            let _ := M.delete (| M.get_subscript (| M.get_field (| M.get_name (| globals, "trie" |), "_data" |), M.get_name (| globals, "key" |) |) |) in
            M.pure Constant.None_
          (* else *)
          )), ltac:(M.monadic (
            M.pure Constant.None_
...
Definition common_prefix_length : Value.t -> Value.t -> M :=
  ...
  |) do
      let _ :=
        ...
        M.if_then_else (|
          BoolOp.or (|
            Compare.gt_e (| M.get_name (| globals, "i" |), M.call (|
              M.get_name (| globals, "len" |),
              make_list [
            ...
            |) |),
            ltac:(M.monadic (
              Compare.not_eq (| M.get_subscript (| M.get_name (| globals, "a" |), M.get_name (| globals, "i" |) |), M.get_subscript (| M.get_name (| globals, "b" |), M.get_name (| globals, "i" |) |) |)
            ))
          |),
        (* then *)
        ltac:(M.monadic (
...
(* At stmt: unsupported node type: AnnAssign *)
  ...
  |) do
      let _ :=
        ...
        (* then *)
        ltac:(M.monadic (
          let _ := M.assert (| Compare.is_not (| M.get_name (| globals, "get_storage_root" |), Constant.None_ |) |) in
          let address :=
            M.call (|
    ...
    make_list [
      Compare.not_eq (| M.get_name (| globals, "encoded_value" |), (* At constant: unsupported node type: Constant *) |);
      M.get_name (| globals, "AssertionError" |)
    ],
    make_dict []
  |) in
...
Definition patricialize : Value.t -> Value.t -> M :=
  fun (args kwargs : Value.t) => ltac:(M.monadic (
      ...
      ltac:(M.monadic (
        let leaf :=
          M.call (|
            M.get_name (| globals, "LeafNode" |),
            make_list [
              M.get_subscript (| M.get_name (| globals, "arbitrary_key" |), M.get_name (| globals, "level" |):(* At expr: unsupported node type: NoneType *) |);
              M.get_subscript (| M.get_name (| globals, "obj" |), M.get_name (| globals, "arbitrary_key" |) |)
            ],
            make_dict []
          |) in
        let _ := M.return_ (|
    ...
    let substring :=
      M.get_subscript (| M.get_name (| globals, "arbitrary_key" |), M.get_name (| globals, "level" |):(* At expr: unsupported node type: NoneType *) |) in
    let prefix_length :=
      M.call (|
        M.get_name (| globals, "len" |),
        make_list [
    ...
    For M.get_name (| globals, "key" |) in M.get_name (| globals, "obj" |) do
        ...
        M.call (|
          ...
          make_list [
            ...
            M.call (|
              ...
              make_list [
                M.get_name (| globals, "substring" |);
                M.get_subscript (| M.get_name (| globals, "key" |), M.get_name (| globals, "level" |):(* At expr: unsupported node type: NoneType *) |)
              ],
              make_dict []
            |)
          ],
      ...
      let _ :=
        (* if *)
        M.if_then_else (|
          Compare.eq (| M.get_name (| globals, "prefix_length" |), Constant.int 0 |),
        (* then *)
        ltac:(M.monadic (
          let _ := M.break (| |) in
          M.pure Constant.None_
    ...
    let _ :=
      (* if *)
      M.if_then_else (|
        Compare.gt (| M.get_name (| globals, "prefix_length" |), Constant.int 0 |),
      (* then *)
      ltac:(M.monadic (
        let prefix :=
          M.get_subscript (| M.get_name (| globals, "arbitrary_key" |), M.get_name (| globals, "level" |):BinOp.add (|
    ...
    For M.get_name (| globals, "key" |) in M.get_name (| globals, "obj" |) do
      let _ :=
        ...
        ltac:(M.monadic (
            ...
            M.if_then_else (|
              M.call (|
                ...
                make_list [
                  ...
                  make_tuple [ M.get_name (| globals, "Account" |); M.get_name (| globals, "Receipt" |); M.get_name (| globals, "Uint" |) ]
                ],
                make_dict []
              |),
            (* then *)
            ltac:(M.monadic (
              let _ := M.raise (| M.get_name (| globals, "AssertionError" |) |) in
              M.pure Constant.None_
            (* else *)
            )), ltac:(M.monadic (
              M.pure Constant.None_
          ...
          let value :=
            M.get_subscript (| M.get_name (| globals, "obj" |), M.get_name (| globals, "key" |) |) in
          M.pure Constant.None_
        (* else *)
        )), ltac:(M.monadic (
          let _ := M.assign (|
          ...
          [M.call (|
            ...
            make_list [
              M.call (|
                M.get_name (| globals, "patricialize" |),
                make_list [
                  M.get_subscript (| M.get_name (| globals, "branches" |), M.get_name (| globals, "k" |) |);
                  BinOp.add (|
                    M.get_name (| globals, "level" |),
                    Constant.int 1
                  |)
