  ...
  |) do
    ...
    make_list [
      M.call (|
        ...
        make_list [
          M.get_subscript (| M.get_name (| globals, "data" |), M.get_name (| globals, "start_position" |):BinOp.add (|
...
Definition Blake2 : Value.t :=
        ...
        " in
          let rounds :=
            M.call (|
              ...
              make_list [
                M.get_subscript (| M.get_name (| globals, "data" |), (* At expr: unsupported node type: NoneType *):Constant.int 4 |)
              ],
              make_dict []
            |) in
          let h :=
          ...
          let f :=
            M.call (|
              M.get_field (| M.get_name (| globals, "Uint" |), "from_be_bytes" |),
              make_list [
                M.get_subscript (| M.get_name (| globals, "data" |), Constant.int 212:(* At expr: unsupported node type: NoneType *) |)
              ],
              make_dict []
            |) in
          let _ := M.return_ (|
      ...
      (
        ...
        " in
          let v :=
            ...
            |) in
          let _ := M.assign (|
            M.get_subscript (| M.get_name (| globals, "v" |), Constant.int 0:Constant.int 8 |),
            M.get_name (| globals, "h" |)
          |) in
          let _ := M.assign (|
            M.get_subscript (| M.get_name (| globals, "v" |), Constant.int 8:Constant.int 15 |),
  ...
  |) do
          ...
          let result_message_words :=
            (BinOp.bit_xor (|
              BinOp.bit_xor (|
                M.get_subscript (| M.get_name (| globals, "h" |), M.get_name (| globals, "i" |) |),
                M.get_subscript (| M.get_name (| globals, "v" |), M.get_name (| globals, "i" |) |)
              |),
              M.get_subscript (| M.get_name (| globals, "v" |), BinOp.add (|
                M.get_name (| globals, "i" |),
                Constant.int 8
              |) |)
